...
export default withApiAuthRequired(async function handler(
  ...
  var url = `${baseURL}/questions/${title}`;
  ...
  if (req.method === "PUT") {
    ...
    if (response.ok) {
      res.status(200).json({ message: `PUT question with title ${restoreTitle(title)}` });
    } else if (response.status === 404) {
      res.status(404).json({ error: "Question not found" });
    } else {
      res.status(response.status).json({ error: "Failed to update question data" });
    }
    ...
    if (response.ok) {
      ...
      res.status(response.status).json({ error: "Failed to delete question" });
    }
  } else {

...
export class DeploymentStack extends cdk.Stack {
    constructor(scope: Construct, id: string, props?: cdk.StackProps) {
        ...
        const questionServiceLambda = new lambda.Function(
            this,
            'questionService',
            {
        ...
        questionResource.addMethod(
            'GET',
            new apigateway.LambdaIntegration(questionServiceLambda, {
                proxy: true,
            })
        );
        questionResource.addMethod(
        ...
        const videoServiceRole = new iam.Role(this, 'VideoServiceRole', {
            roleName: 'video-service-role',
            assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
            description: 'Role that manages Video Service Lambda',
            managedPolicies: [
                iam.ManagedPolicy.fromAwsManagedPolicyName(
                    'service-role/AWSLambdaBasicExecutionRole'
                ),
        ...
        const videoServiceLambda = new lambda.Function(this, 'VideoService', {
            functionName: 'video-service',
            code: lambda.Code.fromBucket(bucket, 'video-service.zip'),
            handler: 'index.handler',
            runtime: lambda.Runtime.NODEJS_18_X,
            role: videoServiceRole,
            environment: {
                ENV: 'PROD',
                APP_ID: 'ead9549ed5af43d5a769644e3136da85',
                APP_CERTIFICATE: '7e5363b5a4a44e8fbf50c7415b763fa9',
            },
        });

        const videoResource = apiGateway.root.addResource('rtc');
        videoResource.addMethod(
            'GET',
            new apigateway.LambdaIntegration(videoServiceLambda, {
                proxy: true,
            })
        );
        videoResource.addProxy({
            defaultIntegration: new apigateway.LambdaIntegration(
                videoServiceLambda
            ),
            defaultMethodOptions: {
